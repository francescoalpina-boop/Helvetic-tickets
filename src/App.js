...
function QRCode({ data, size }) {
  ...
  return (
    <div style={{ background: "#fff", padding: 8, borderRadius: 8, display: "inline-block" }}>
      <QRCodeSVG value={data} size={sz} level="M" />
    </div>
  );
}
...
function HDCLogo({ size }) {
  ...
  return (
    <svg width={w} height={h} viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">
      <text x="160" y="52" textAnchor="middle" fontFamily="Georgia,serif" fontSize="38" fill="#1a5c1a" fontStyle="italic">Helvetic</text>
      <text x="160" y="122" textAnchor="middle" fontFamily="Georgia,serif" fontSize="72" fill="#1a5c1a" fontStyle="italic">Dance</text>
      <text x="160" y="168" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="26" fill="#1a5c1a" fontWeight="900" letterSpacing="4">COMPETITION</text>
    </svg>
  );
}
...
function EllevelLogo({ size }) {
  ...
  return (
    <svg width={w} height={h} viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">
      <text x="160" y="85" textAnchor="middle" fontFamily="Georgia,serif" fontSize="42" fill="#8b0000" fontStyle="italic">Wellness</text>
      <text x="160" y="145" textAnchor="middle" fontFamily="Georgia,serif" fontSize="72" fill="#8b0000" fontStyle="italic">Ellevel</text>
      <text x="160" y="178" textAnchor="middle" fontFamily="Arial,sans-serif" fontSize="22" fill="#8b0000" fontWeight="700" letterSpacing="4">FITNESS</text>
    </svg>
  );
}
...
function TicketModal({ event, onClose }) {
  ...
  function handlePay() {
    ...
    setTimeout(async function() {
      ...
      var lines = event.tickets.filter(function(_, i) { return qtys[i]; }).map(function(t, i) { return t.type + " x" + (qtys[i] || 1); });
  ...
  return (
    ...
    >
      <div style={{ background: "#0f0f13", borderRadius: 24, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto", border: "1px solid rgba(255,255,255,0.08)" }}>
        ...
        {/* Header */}
        <div style={{ background: "linear-gradient(135deg," + C + "22," + A + "11)", padding: "1.5rem", borderBottom: "1px solid rgba(255,255,255,0.06)", position: "relative" }}>
          ...
          {step === "payment" && (
            <div>
              ...
              <button onClick={handlePay} style={{
                marginTop: 10, width: "100%", padding: "0.7rem",
                background: "rgba(255,255,255,0.06)",
                border: "1px dashed rgba(255,255,255,0.2)",
                borderRadius: 12, color: "rgba(255,255,255,0.4)",
                fontSize: "0.82rem", cursor: "pointer"
              }}>
                🧪 Modalità test — genera QR senza pagare
              </button>
            </div>
          )}
...
function EventCard({ event, onBuy }) {
  ...
  return (
    ...
    >
      {/* Banner */}
      <div style={{ background: event.id === 2 ? "linear-gradient(135deg,#fde8e8,#fff0f0)" : "linear-gradient(135deg," + C + "33," + A + "22)", padding: "2rem", textAlign: "center", position: "relative", borderBottom: "1px solid " + C + "22" }}>
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08080c", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      <link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"/>
